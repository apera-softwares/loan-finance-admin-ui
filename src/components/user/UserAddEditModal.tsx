...
const UserAddEditModal: React.FC<UserAddEditModalProps> = ({
  ...
  const [showSalesRepDropdown, setShowSalesRepDropdown] =
  ...
  useEffect(() => {
    if (userData) {
      setFormData({
        ...
        availableCredit:
          userData?.UserDetails?.[0]?.availableCredit?.toString() || "",
        interestRate:
          userData?.UserDetails?.[0]?.interestRate?.toString() || "",
        ...
        utilizedCredit:
          userData?.UserDetails?.[0]?.utilizedCredit?.toString() || "",
      });
  ...
  const fetchSalesRep = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${BACKEND_API}sales-reps?page=1&limit=20&name=${salesRepName}`,
        {
          headers: {
  ...
  const handleEdit = () => {
    ...
    const payload = {
      ...
      salesRepUserId:
        formData.salesRepUserId ||
        userData?.UserDetails?.[0]?.salesRepUserId ||
        undefined,
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="relative w-full" ref={salesRepDropdownRef}>
                ...
                <input
                  ...
                  value={
                    selectedSalesRep
                      ? `${selectedSalesRep?.firstName || ""} ${
                          selectedSalesRep?.lastName || ""
                        }`
                      : ""
                  }
              ...
              {type === "update" && (
                <>
                  <div className="w-full">
                    <label className={MODAL_INPUT_LABEL_CLASS}>Status</label>
                    <select
                      name="status"
